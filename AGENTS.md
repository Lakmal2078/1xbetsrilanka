...
This document provides an overview of the **1xBet Fast Cash** project structure for developers and AI agents working on this codebase.

## Project Overview

A single-page promotional/marketing landing page for 1xBet Fast Cash — a Sri Lanka VIP Cash Agent. Content is primarily in Sinhala (සිංහල) with English labels. Built with TanStack Start and deployed on Netlify.
...
src/
  routes/
    __root.tsx    — Root shell: <html>, head meta/links, Poppins + Font Awesome CDN
    index.tsx     — Full landing page (single route, all sections)
  styles.css      — Global styles, Tailwind v4 @theme tokens, animations
...
All page content lives in one route: `src/routes/index.tsx`.

Sections in order: Nav → Hero → Stats → Features → Bonus → Steps → Footer.

- Smooth scroll navigation via `document.getElementById().scrollIntoView`.
- IntersectionObserver drives `.card-animate` → `.visible` entrance animations.
- Font Awesome loaded via CDN link in `__root.tsx` head (avoids bundling full icon set).
- Hero background uses inline SVG data URI (no external image dependency).
- Tailwind v4 `@theme` block in `styles.css` defines `--color-primary/secondary/dark`.
- Animation classes (`floating`, `pulse`, `fade-in-up`, `card-animate`) are in `styles.css`.

## Conventions

- Components: PascalCase
- Routes: file-based in `src/routes/`
- Styling: Tailwind utility classes; custom animations in `styles.css`
